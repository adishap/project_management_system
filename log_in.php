...
if(isset($_POST['log_in'])){
	...
	if($query_run = mysqli_query($con,$email_check_project_query)){
      $query_num_rows = mysqli_num_rows($query_run);
      
      if($query_num_rows == 0){//if no then check if email exists in peers table
      
      	$email_check_project_query = "SELECT * FROM `peers` WHERE `user_email` = '".$user_email."' AND `password` = '".$password."'";
		if($query_run = mysqli_query($con,$email_check_project_query)){
      		$query_num_rows = mysqli_num_rows($query_run);
      		if($query_num_rows == 0){
